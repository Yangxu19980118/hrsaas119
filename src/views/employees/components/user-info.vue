<template>
  <div class="user-info">
    ...
    <el-row type="flex" justify="end" style="margin-bottom:20px">
      <el-button type="primary" icon="el-icon-printer" size="small" @click="$router.push(`/employees/print/${userId}?type=personal`)">打印</el-button>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    async savePersonal() {
      const fileList = this.$refs.baseInfor.fileList
      if (!fileList.length) {
        return this.$message.warning('上传员工照片')
      }
    ...
    async saveUser() {
    //  调用父组件
      const fileList = this.$refs.userInfor.fileList
